...
const MobileMenu = (props) => {
  const { isOpen, toggleMenu, path } = props;
      ...
      >
        <ul className="self-start flex flex-col gap-3 ml-5 p-6 space-y-4 mt-20 list-none">
          ...
          <li>
            <Link
              to="/contact"
              className={`text-[#3F404AB2] no-underline hover:text-[#4D0B5E] ${path.includes("contact"
              ) && "text-[#4D0B5E]"}`}
              onClick={toggleMenu}
            >
              CONTACT
        ...
        <button className="w-[70%] h-[4rem] text-base md:w-[90px] min-h-[30px] font-[500] text-[#4D0B5E] bg-white border border-[#4D0B5E] rounded-[8px] font-[cabin] shadow-evenly">
          SIGN IN
        </button>
        <FaTimes
          onClick={toggleMenu}
