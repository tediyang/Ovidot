...
import { FaShieldAlt, FaUserSlash, FaSpinner, FaLock } from 'react-icons/fa';
...
const SectionHeader = ({ label }) => (
  <div className="flex items-center gap-3 mb-5">
    <h3 className="text-sm font-bold text-primary whitespace-nowrap">{label}</h3>
...
const AdminSettingsPage = () => {
  ...
  // Switch role state
  const [switchForm, setSwitchForm] = useState({ email_username_id: '', role: 'ADMIN', loading: false });

  // Deactivate state
  const [deactivateForm, setDeactivateForm] = useState({ email_username_id: '', loading: false, confirm: false });
  ...
  const handleSwitchRole = async (e) => {
    ...
    try {
      await adminApiService.switchRole(switchForm.email_username_id.trim(), switchForm.role);
  ...
  const handleDeactivate = async (e) => {
    ...
    try {
      await adminApiService.deactivateAdmin(deactivateForm.email_username_id.trim());
      flashToast('Admin deactivated successfully.');
      setDeactivateForm({ email_username_id: '', loading: false, confirm: false });
    } catch (err) {
        ...
        <div className="flex flex-col m-4 mt-14 lg:min-w-[45rem] xl:min-w-[62rem] lg:ml-[16rem] gap-5">
          ...
          <div className="bg-white rounded-2xl shadow-[0_2px_12px_rgba(77,11,94,0.07)] p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-bold text-gray-800 text-lg">{username[0].toUpperCase() + username.slice(1)}</h3>
                <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold mt-1 ${
                  isSuperAdmin ? 'bg-primary/10 text-primary' : 'bg-violet-100 text-violet-700'
                }`}>
                  {role}
                </span>
              </div>
          ...
          {!isSuperAdmin && (
            <div className="bg-white rounded-2xl shadow-[0_2px_12px_rgba(77,11,94,0.07)] p-6 border-l-4 border-amber-400">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-800">Super Admin required</p>
                  <p className="text-xs text-gray-500 mt-0.5">
                    Admin management actions (switching roles, deactivating admins) are only available to Super Admins.
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* Switch Role */}
          <div className={`bg-white rounded-2xl shadow-[0_2px_12px_rgba(77,11,94,0.07)] p-6 ${!isSuperAdmin ? 'opacity-50 pointer-events-none select-none' : ''}`}>
            <SectionHeader label="Switch Admin Role" />
            <p className="text-xs text-gray-400 mb-5">
              Change the role of an existing admin account. Provide their email address, username, or ID.
            </p>
            <form onSubmit={handleSwitchRole} className="flex flex-col gap-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500">Email / Username / ID</label>
                  <input
                    type="text"
                    value={switchForm.email_username_id}
                    onChange={e => setSwitchForm(p => ({ ...p, email_username_id: e.target.value }))}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500">New Role</label>
                  <select
                    value={switchForm.role}
                    onChange={e => setSwitchForm(p => ({ ...p, role: e.target.value }))}
                    className="px-3 py-2.5 border-[1.5px] border-solid border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 transition-colors"
                  >
          ...
          {/* Deactivate Admin */}
          <div className={`bg-white rounded-2xl shadow-[0_2px_12px_rgba(77,11,94,0.07)] p-6 border-l-4 border-red-300 ${!isSuperAdmin ? 'opacity-50 pointer-events-none select-none' : ''}`}>
            <SectionHeader label="Deactivate Admin Account" />
            <p className="text-xs text-gray-400 mb-5">
              Deactivate an admin account so they can no longer sign in. This cannot deactivate a Super Admin.
            </p>
            <form onSubmit={handleDeactivate} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5 max-w-sm">
                ...
                <input
                  ...
                  onChange={e => setDeactivateForm(p => ({ ...p, email_username_id: e.target.value, confirm: false }))}
                  placeholder="admin@example.com"
              ...
              {deactivateForm.confirm && (
                <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-700">
                  ...
                  <p className="text-xs mt-0.5">This will deactivate the admin account. Click the button again to confirm.</p>
                </div>
              )}
              ...
              <button
                ...
                className={`${deactivateForm.loading ? 'opacity-60 cursor-not-allowed' : 'hover:opacity-90'} flex items-center gap-2 w-fit px-6 py-2.5 ${
                  deactivateForm.confirm ? 'bg-red-600' : 'bg-red-500'
                } text-white text-sm font-semibold rounded-xl border-0 cursor-pointer transition-all`}
              >
                {deactivateForm.loading
                  ? <><FaSpinner className="animate-spin" size={13} /> Processing…</>
                  : <><FaUserSlash size={13} /> {deactivateForm.confirm ? 'Confirm Deactivation' : 'Deactivate Admin'}</>
                }
              </button>
            </form>
          </div>
