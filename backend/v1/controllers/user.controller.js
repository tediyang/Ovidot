...
const {
  ...
  User,
  MongooseError,
} = require("../models/engine/database.js");
...
class UserController {
    ...
    try {
      ...
      if (value.dob > minDOB || value.dob < maxDOB) {
        return handleResponse(
          res,
          400,
          "You are too young or above the age to menstrate (min: 8 years, max: 58 years)",
        );
      }

      const { fname, lname, dob, period, username, sensitive, password } =
        value;
      ...
      if (fname) {
        ...
        message += message === "" ? "first name": ", first name"
      }
      if (lname) {
        user.name.lname = lname;
        message += message === "" ? "last name": ", last name"
      }
      if (dob) {
        user.dob = dob;
        message += message === "" ? "date of birth": ", date of birth"
      }
      if (period) {
        user.period = period;
  ...
  async readAllNotifications(req, res) {
    try {
      ...
      user.notificationsList.map((note) => {
        note.status = notificationStatus.read;
      });
  ...
  async deleteNotification(req, res) {
    try {
      ...
      user.notificationsList = user.notificationsList.filter(
        (data) => data._id !== note._id,
      );
