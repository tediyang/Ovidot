...
class AppController {
   ...
   */
    async createToken(payload, just_access=false) {
      try {
        if(just_access) {
          const accessToken = sign({ id: payload._id, email: payload.email, status: payload.status }, secretKey, { expiresIn: '5h' });
          return accessToken;
        }
        const accessToken = sign({ id: payload._id, email: payload.email, status: payload.status }, secretKey, { expiresIn: '5h' });
        const refreshToken = sign({id: payload._id}, secretKey, { expiresIn: '7d' });
 ...
 */
  async refreshToken(req, res) {
    try {
      ...
      const user = await User.findById(userPayload.id, 'jwtRefreshToken email _id status').exec();
			if(!user) {
      ...
      if(user.jwtRefreshToken !== token) {
        return handleResponse(res, 400, "Invalid Credential, Refresh token invalid");
      }
