...
class CycleController {
    ...
    try {
      ...
      if (cycleHelper.checkExistingCycle(user, startdate)) {
        return handleResponse(
          res,
          400,
          "Cycle already exist for this month: Delete to create another",
        );
      }

      // if period wasn't given
      if (!period) {
        // if user.period is not set, then throw error
        if (!user.period) {
          return handleResponse(
            res,
            400,
            "Period is required if you haven't set a default period in your profile",
          );
        }
        period = user.period;
      }
      ...
      const cycleData = await cycleCalculator.calculate(
        period,
        startdate,
        ovulation,
      );
      ...
      await Promise.resolve(
        redisManager.cacheDel(id, newCycle.year.toString()),
      );
    ...
    try {
      ...
      if (cycles && cycles !== "[]") {
        ...
        const filtered =
          month && period
            ? parsedCycles.filter(
                (cycle) => cycle.month === month && cycle.period == period,
              )
            : month
              ? parsedCycles.filter((cycle) => cycle.month === month)
              : period
                ? parsedCycles.filter((cycle) => cycle.period == period)
                : JSON.parse(cycles);
  ...
  async fetchOneCycle(req, res) {
    try {
      ...
      const user = await userPopulate.populateWithCyclesBy(userId, {
        _id: cycleId,
      });
      if (user === null) {
        return handleResponse(res, 404, "User not found");
    ...
    try {
      ...
      if (
        ...
        return handleResponse(
          res,
          400,
          "Can't update the cycle with the same period and ovulation",
        );
      }
      if (period && !ovulation && period === cycle.period) {
        return handleResponse(
          res,
          400,
          "Can't update the cycle with the same period",
        );
      }
      if (
        !period &&
        ...
        return handleResponse(
          res,
          400,
          "Can't update the cycle with the same ovulation",
        );
      }
      ...
      if (
        new Date() >
        new Date(cycle.start_date).setDate(
          new Date(cycle.start_date).getDate() + 30,
        )
      ) {
        return handleResponse(
          res,
          400,
          "Update can't be made after 30 days from current cycle start date",
        );
      }
      ...
      if (
        ovulation &&
        !dateValidator.validateUpdateDate(
          cycle.start_date,
          ovulation,
          cycle.period,
        )
        ...
        return handleResponse(
          res,
          400,
          "Ovulation date must not exceed 18 days from start date",
        );
      }
      ...
      const updatedCycle = await cycleHelper.performUpdateAndNotify(
        cycle,
        period,
        ovulation,
        cycleId,
      ...
      // Handle cache
      await Promise.resolve(
        redisManager.cacheDel(userId, updated.year.toString()),
      );
  ...
  async deleteCycle(req, res) {
    try {
      ...
      // Handle cache
      await Promise.resolve(
        redisManager.cacheDel(userId, deletdCycle.year.toString()),
      );
