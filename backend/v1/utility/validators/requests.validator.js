...
class Validator {
  ...
  UpdateUser = Joi.object({
    ...
    if (sensitive && !password) {
      return helpers.error(
        "Password is required for updating sensitive fields (phone and password)",
      );
    }
    return value;
  });
  ...
  CreateCycle = Joi.object({
    period: Joi.number().min(2).max(8),
    startdate: Joi.date() // YYYY-MM-DD
      .required(),
    ovulation: Joi.date(), // YYYY-MM-DD
  });
  ...
  GetUsers = Joi.object({
    ...
    createdAt: Joi.object({
      ...
      .messages({
        "exact_range.invalid_length":
          "You must provide not more than two dates for exact_range",
        "range_and_exact_range.conflict":
          "You cannot set both range and exact_range",
      }),
