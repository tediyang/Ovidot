...
const { HOST, REDIS_URL, ENVIR, PORT } = process.env;
...
const startServer = async (app) => {
  ...
  app.listen(port, HOST, () => {
    logger.info(`Server listening securely on HOST ${HOST} and PORT ${port}`);
    logger.info(
      "Email cron execution is handled by an external service via /email-cron.",
    );
  });
};
